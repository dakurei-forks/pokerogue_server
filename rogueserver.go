...
func main() {
	...
	if *vouchergift {
		...
		var userUuids [][]byte
		var userUsernames []string

		if *vouchergiftall {
			log.Printf("Voucher Gift for everybody")
			userUuids, userUsernames, err = db.FetchAllUUIDAndUsername()
			if err != nil {
			...
			userUsernames = append(userUsernames, *username)
		}
		...
		for i := 0; i < len(userUuids); i++ {
			...
			log.Printf("  Gifted to %s", userUsernames[i])
		}
